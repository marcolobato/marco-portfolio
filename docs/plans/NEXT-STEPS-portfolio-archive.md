...
- [ ] **Compress the remaining oversized videos** under [public/projects/](public/projects/) for faster page loads. `das-experience-concept.mp4` was already compressed from 37 MiB → 19 MiB to clear Cloudflare's 25 MiB per-file limit, but several others are still bigger than they need to be: `das-functional-prototype.mp4` (16 MiB), `vma-process-in-car-testing.mp4` (9 MiB), `ma-otto-interface.mp4` (6.2 MiB), `privacysim-session.mp4` (6.3 MiB). Suggested approach: `ffmpeg -i in.mp4 -c:v libx264 -crf 28 -preset slow -c:a aac -b:a 96k -movflags +faststart out.mp4` — typically halves the size with no visible quality loss at 720p. Not blocking deploys, but a noticeable load-speed improvement, especially on mobile/slow connections.
